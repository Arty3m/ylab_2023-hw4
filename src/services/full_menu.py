...
class FullMenu(ServiceMixin):
    async def create_all_menu(self) -> dict:
        ...
        await add_to_db(self.db, menu_1)
        ...
        await add_to_db(self.db, submenu_1_1)
        dish_1_1: Dish = Dish(
        ...
        await add_to_db(self.db, dish_1_1)
        await add_to_db(self.db, dish_1_2)
        submenu_1_2: SubMenu = SubMenu(**{'title': 'Рамен', 'description': ' Горячий рамен'})
        submenu_1_2.owner = menu_1.id
        submenu_1_2.dishes_count = 3
        await add_to_db(self.db, submenu_1_2)
        dish_1_2_1: Dish = Dish(
            **{'title': 'Дайзу рамен', 'description': 'Рамен на курином бульоне с куриными подушками и яйцом',
               'price': '166.87'})
        ...
        await add_to_db(self.db, dish_1_2_1)
        await add_to_db(self.db, dish_1_2_2)
        await add_to_db(self.db, dish_1_2_3)
        ####################################
        menu_2: Menu = Menu(**{'title': 'Алкоголь', 'description': 'Алкогольные напитки'})
        menu_2.submenus_count = 2
        menu_2.dishes_count = 6

        await add_to_db(self.db, menu_2)
        ...
        await add_to_db(self.db, submenu_2_1)
        ...
        await add_to_db(self.db, dish_2_1_1)
        await add_to_db(self.db, dish_2_1_2)
        await add_to_db(self.db, dish_2_1_3)
        ...
        await add_to_db(self.db, submenu_2_2)
        dish_2_2_1: Dish = Dish(
            **{'title': 'Джемисон', 'description': 'Классический купажированный виски, 4х летней выдержки',
               'price': '540.22'})
        dish_2_2_1.owner = submenu_2_2.id
        dish_2_2_2: Dish = Dish(**{'title': 'Джек Дениелс',
        ...
        dish_2_2_3: Dish = Dish(**{'title': 'Чивас Ригал',
                                   'description': 'Это купаж высококачественных солодовых и зерновых виски, выдержанный'
                                                  ' как минимум в течение 12 лет', 'price': '750.25'})
        dish_2_2_3.owner = submenu_2_2.id
        await add_to_db(self.db, dish_2_2_1)
        await add_to_db(self.db, dish_2_2_2)
        await add_to_db(self.db, dish_2_2_3)
...
def get_create_full_menu(db: AsyncSession = Depends(get_db), cache=Depends(get_redis)) -> FullMenu:
    return FullMenu(db=db, cache=cache)
