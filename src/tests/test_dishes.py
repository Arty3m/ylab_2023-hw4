...
class TestDishes:
    ...
    def test_get_dishes_empty(self, test_app):
        response = test_app.get(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes')
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == []

    def test_create_dishes(self, test_app):
        test_dish = {'title': 'Dish 1', 'description': 'Dish description 1', 'price': '12.555'}
        response = test_app.post(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes',
    ...
    def test_get_dish_by_id(self, test_app):
        response = test_app.get(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}')
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == {'id': dish_id, 'title': 'Dish 1', 'description': 'Dish description 1',
                                   'price': '12.55'}

    def test_get_dish_by_incorrect_id(self, test_app):
        response = test_app.get(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/10000')
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.json() == {'detail': 'dish not found'}

    def test_patch_dish_by_id(self, test_app):
        test_updated_dish = {'title': 'Updated dish 1', 'description': 'Updated dish description 1', 'price': '333'}
        response = test_app.patch(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}',
    ...
    def test_patch_dish_by_incorrect_id(self, test_app):
        test_updated_dish = {'title': 'Updated dish 1', 'description': 'Updated dish description 1', 'price': '333'}
        response = test_app.patch(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/10000',
                                  content=json.dumps(test_updated_dish))
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.json() == {'detail': 'dish not found'}

    def test_delete_dish(self, test_app):
        response = test_app.delete(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}')
        assert response.status_code == status.HTTP_200_OK
