...
def test_get_menu_by_id(test_app):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.json() == {"id": menu_id, "title": "Menu 1", "description": "Menu description 1",
                               "submenus_count": 0,
                               "dishes_count": 0}
...
def test_patch_menu_by_id(test_app):
    test_update_data = {"title": "Updated menu 1", "description": "Updated menu description 1"}
    response = test_app.patch(f'/api/v1/menus/{menu_id}', content=json.dumps(test_update_data))
    assert response.status_code == status.HTTP_200_OK
...
def test_patch_menu_by_incorrect_id(test_app):
    test_update_data = {"title": "Updated menu 1", "description": "Updated menu description 1"}
    response = test_app.patch(f'/api/v1/menus/10000', content=json.dumps(test_update_data))
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {"detail": "menu not found"}
...
def test_del_menu_not_exist(test_app):
    response = test_app.delete(f'/api/v1/menus/10000')
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {'detail': "menu not found"}
